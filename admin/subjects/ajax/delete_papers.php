...
try {
    ...
    // Delete previous papers
    $stmt = $conn->prepare("DELETE FROM previous_papers WHERE subject_code = ?");
    $stmt->execute([$subject['subject_code']]);

    header('Content-Type: application/json');
    echo json_encode(['success' => true, 'message' => 'Previous year papers deleted successfully']);
